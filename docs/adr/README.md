...
> 2026-09-21 모노레포에서 이 레포로 분리하며 히스토리를 재작성해 옛 커밋 해시가 무효가 되었다. 아래 표는 해시 대신 커밋 제목으로 식별한다(`git log --grep`).

아래는 ADR 기준을 만족할 수 있는 과거 결정이지만, **작성자가 당시 맥락·대안·감수한 점을 직접 확인해 채워야 한다**(추측으로 근거를 지어내지 않기 위해 여기 후보로만 남긴다).

| 결정 | 시점(커밋 제목) | 메모 |
| --- | --- | --- |
| 메일 발송을 Gmail SMTP → AWS SES 자체 도메인으로 전환 | 2026-08-21 `infra: 메일 발송을 Gmail에서 SES 자체 도메인으로 전환` | 도달성·발송 한도·자체 도메인 인증(SPF/DKIM) 근거 필요 |
| 알림 fan-out을 키워드 구독만으로 단일화 | 2026-07-29 `feat: 알림 발송을 키워드 구독만으로 fan-out` | 토픽 구독은 알림 미발송으로 둔 이유·topic_subscription 도먼트 보존 |
| 로그인 없는 비회원 이메일 구독 + notification 다형화 | 2026-08-23 `feat: 인증 비회원 이메일 구독자 다이제스트 발송, 비회원 수신거부` | member XOR email_subscriber, claim 시 re-key 결정 근거 |
| 회원당 알림 최대 5개 상한 | 2026-07-20 `feat: 알림을 회원별 최근 수집순 5개로 제한` | 상한 값·최근 수집순 기준 근거 |
